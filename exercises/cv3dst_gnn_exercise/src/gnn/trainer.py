...
def train_one_epoch(model, data_loader, optimizer, accum_batches=1, print_freq=200):
    ...
    for i, batch in tqdm.tqdm(enumerate(data_loader)):
        ...
        for sample in batch:
            ...
            track_feats, track_coords, track_ids = past_frame['features'].to(device), past_frame['boxes'].to(device), \
            past_frame['ids'].to(device)
            current_feats, current_coords, curr_ids = curr_frame['features'].to(device), curr_frame['boxes'].to(device), \
            ...
            loss = F.binary_cross_entropy_with_logits(assign_sim, same_id, pos_weight=torch.as_tensor(20.)) / float(
                len(batch))
            loss.backward()
        ...
        if (i + 1) % print_freq == 0 and i > 0:
            log_str = ". ".join(
                [f"{m_name.capitalize()}: {m_val / (print_freq if i != 0 else 1):.3f}" for m_name, m_val in
                 metrics_accum.items()])
